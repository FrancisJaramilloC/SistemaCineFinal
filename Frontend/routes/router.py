...
def view_edit_person(id):  
    ...
    r1 = requests.get(f"http://localhost:8099/myapp/person/get/{id}")  
    data1 = r1.json()
...
def save_person():
    ...
    r = requests.post("http://localhost:8099/myapp/person/save", data=json.dumps(dataF), headers=headers)
    dat = r.json()
...
def delete_person(id):
    ...
    r = requests.delete(f"http://localhost:8099/myapp/person/delete/{id}")
...
def update_person():
    ...
    r = requests.post("http://localhost:8099/myapp/person/update", data=json.dumps(dataF), headers=headers)
    print(r)
    dat = r.json()
    # Manejo de la respuesta
    if r.status_code == 200:
